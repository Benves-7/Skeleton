...
public:
	...
	void modifyVertexShader(Matrix4D mat);
	void modifyFragmentShader(Vector4D vec);
	void useProgram();
	void del();
...
inline void ShaderObject::modifyVertexShader(Matrix4D mat)
{
	mat.GetArr();
...
inline void ShaderObject::modifyFragmentShader(Vector4D vec)
{
	unsigned int transformLoc = glGetUniformLocation(mProgramId, "transform");
	glUniformMatrix4fv(transformLoc, 1, GL_TRUE, vec.GetPointer());
}
