...
{
	Vector4D cameraDirection = (cameraPosition - cameraTarget).Normalize();
	Vector4D cameraRight = Vector4D::cross(cameraUp, cameraDirection).Normalize();
	Matrix4D temp1 = Matrix4D(
		cameraRight.GetX(), cameraRight.GetY(), cameraRight.GetZ(), 0,
		cameraUp.GetX(), cameraUp.GetY(), cameraUp.GetZ(), 0,
		cameraDirection.GetX(), cameraDirection.GetY(), cameraDirection.GetZ(), 0,
		0, 0, 0, 1);
	Matrix4D temp2 = Matrix4D(
