...
public:
	...
	inline vector<Vertex> getVBuffer() { return vbuffer; }
	inline vector<unsigned int> getIBuffer() { return ibuffer; }
...
inline bool MeshResource::loadOBJinternal(const char* path, vector<Vertex> &vbuffer, vector<unsigned int> &ibuffer)
{
	...
	printf("Done reading! \n");
	for each (FaceVertex face in faceList)
	{
		...
		v.uv[0] = temp_Texture[face.uv - 1].GetX();
		v.uv[1] = temp_Texture[face.uv - 1].GetY();
